 ...
 * @since 1.4 Mejorado los métodos comprobarEntero() y comprobarFloat()
...
// Return false no es correcto, true es correcta
    ...
    /**
     * Basada en validarAlfanumerico, esta función comprueba si el nombre de un
    ...
    public static function validarColorHex($colorHex, $obligatorio = 0) {
        ...
        if (empty($colorHex)) { //Compruebo si la variable es vacía
            if ($obligatorio == 1) { //Aqui si el valor es '1' es obligatorio y devolveremos un mensaje
                return "Debes ingresar un color.";
            } else {
                return null; //Aqui ll color es opcional y no se ingresó
            }
        }

        if (preg_match($patron, $colorHex)) { //Si la variable no está vacía, por medio del método 'preg_match' verificamos si la cadena cumple con el patrón.
            return null; //Devolvemos 'null' si el color es válido, si no un mensaje de error.
        } else {
